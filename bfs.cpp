...
queue<int>q;                                        //队列存放未检查的点
int Source,End,fa[N];                              //Source为源点，End为终点，fa用于记录路径，从终点沿着fa回溯到起始点。
stack<int>path;                                   //path用于保存路径，用栈可以正序输出最终变换步数
bool bfs(){
    q.push(Source);                     
    dis[Source] = 0;
    while(!q.empty()){
        int cur=q.front();                 //取出队列中第一个点
        if(cur==End)return true;          //若已经找到路径到达终点直接返回
        q.pop();
        if(vis[cur])continue;
        vis[cur]=1;
        for(int i=0;i<store[cur].size();i++){
            int v=store[cur][i];
            if(dis[v]>dis[cur]+1&&!vis[v]){     //若v状态从cur状态转移所需步数更少，则让v从cur转移
                fa[v]=cur;
                dis[v]=dis[cur]+1;
                q.push(v);
            }
...
int main(){
    ...
    while(true){
        initial();
        printf("Please enter a 9-digit number(0 means the space) as you origin grid and -1 to exit:\n");
        scanf("%d",&Source);
        if(Source==-1){
