...
*/
const int N=2E6+10;int n; double k=0.0;                 //只有362880种状态，每种状态最多可以向4个方向转移,4*362880=1,451,520，N为2E6则可。
//Part 1:-----------------------------------------//A*算法计算单源最短路
...
void pre(){
    ...
    //求取每个点的H；设置每个点的G为1，即从i点到j点的连接都是加上1
   ...
   //cout<<"ENDPOSITION\n";
    for(int i=0;i<9;i++){
        ...
        //cout<<EndPosition[i].first<<' '<<EndPosition[i].second<<endl;
    }
    //cout<<"endend\n";
    for(int i=1;i<=n;i++){
        //cout<<match[i]<<' '<<endl;
        for(int j=0;j<9;j++){
            PII TempPosition=ReflectToNumberPosition(i,j);
           // cout<<TempPosition.first<<' '<<TempPosition.second<<endl;
            node[i].H+=abs(TempPosition.first-EndPosition[j].first)+abs(TempPosition.second-EndPosition[j].second);
        }
    }
}
...
int main(){
    ...
    //freopen("StoreAStar.txt","w",stdout);
    pre();
    clock_t  start,stop;
    start=clock();
